...
and add it to path using Modules/modulecmd:
    ...
    meson build --prefix /CAD_DISK/verilator/`git describe | sed "s/verilator_//"`
...
most GNU tools support:
    ...
    unset VERILATOR_ROOT      # if your shell is bash
    unsetenv VERILATOR_ROOT   # if your shell is csh
    meson build --prefix /opt/verilator-VERSION
...
Type C<ninja> to compile Verilator.

Type C<ninja test> to check the compilation.
...
If you used the prefix scheme, now do a C<ninja install>.  To run verilator,
...
You may now wish to consult the examples directory. Type C<ninja> inside any
